...
const getProductController = async(req, res)=>{
    try {
        ...
        if(!getProductData){
            return res.status(404).json({success:false, error:"product items not found"});
        }

        return res.json({success:true, data:getProductData});
    } catch (error) {
...
const getProductByIdController = async(req, res) =>{
    try {
        ...
        if(!getByIdData || getByIdData === undefined){
            return res.status(404).json({success:false, error:"product id not found"});
        }
        
        return res.json({success:true, data:getByIdData});
    } catch (error) {
        return res.status(500).json({success:false, error:"Server Error"});
...
const createProductController = async(req, res) =>{
    try {
        ...
        return res.status(201).json({success:true, message:"Creat new product successful"});
    } catch (error) {
        return res.status(500).json({success:false, error:"Server Error"});
    }
};
...
const editProductController = async(req, res) =>{
    try {
        ...
        return res.json({success:true, message:"product data updated"});
    } catch (error) {
        return res.status(500).json({success:false, error:"Server error"});
    }
};
...
const deleteProductController = async(req, res) =>{
    try {
        ...
        return res.json({success:true, message:"delete the product data"});
    } catch (error) {
        return res.status(500).json({success:false, error:"Server error"});
    }
}
